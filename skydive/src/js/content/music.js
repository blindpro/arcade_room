// SKYDIVE music: an upbeat 8-bit bed that intensifies as your fall speeds up —
// the longer you go without a crystal, the faster you fall and the harder the
// music pushes you to catch one. A sixteenth-note sequencer scheduled on the
// audio clock with a short lookahead (frame/setTimeout jitter never causes
// audible gaps); the game loop only refills the queue via update(). Intensity
// `intensity` in [0,1] comes from fallSpeed / MAX_FALL_SPEED and drives tempo,
// layers and brightness. Mixed under the crystal cues so the spatial pings
// always read on top. Started on game-screen enter, stopped on exit (so pausing
// stops it cleanly). Self-contained: no reach into content.audio's privates.
content.music = (() => {
  const LOOKAHEAD = 0.16 // seconds scheduled ahead of the audio clock
  const STEPS = 16       // one bar of sixteenth notes

  // A i–VI–VII–i vamp in A minor, one chord per bar.
  const BARS = [
  ...
  function bpm() { return Math.min(168, 104 + intensity * 64) }
  ...
  function scheduleStep(s, t) {
    ...
    // square bass groove
    if (intensity > 0.15) {
      const eighth = (s === 0 || s === 2 || s === 4 || s === 6 || s === 8 || s === 10 || s === 12 || s === 14)
      if (eighth) {
    ...
    // chiptune lead arpeggio — chord tones cycling, busier as you fall faster
    if (intensity > 0.25) {
