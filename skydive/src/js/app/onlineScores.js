 ...
 *
 * REGISTERED as game id `skydive` (display_order: desc, max_score: 9999999,
 * meta_schema: [{"key":"crystals","type":"int","min":0,"max":999}]). See
 * /home/scores/INTEGRATION.md for the contract. The SECRET below is a
 * placeholder — replace it with the real one when the game is registered on
 * scores.oriolgomez.com; until then submissions fail gracefully and scores
 * stay local.
 */
app.onlineScores = (() => {
  const BASE = 'https://scores.oriolgomez.com'
  const GAME_ID = 'skydive'
  const SECRET = 'skydive-placeholder-secret-not-registered-yet'
