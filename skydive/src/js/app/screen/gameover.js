app.screen.gameover = app.screenManager.invent({
  ...
  onEnter: function () {
    ...
    const crystals = content.game.state.crystals
  ...
  handleSave: function () {
    ...
    const score = content.game.state.score
    const crystals = this.state._crystals || 0
    ...
    if (this.state.qualifies) app.highscores.add(name, score, crystals)
    ...
    Promise.resolve(app.onlineSubmit.run({
      name: name, score: score, meta: {crystals: crystals},
