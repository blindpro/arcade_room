;(async () => {
  ...
  // This game authors its own short per-cue tails and depends on a clean stereo
  // image for the positional pan cue, so kill the global reverb send.
  if (engine.mixer.reverb && engine.mixer.reverb.setActive) {
    engine.mixer.reverb.setActive(false)
  }

  // Autoplay policy starts the context suspended; resume it on the first user
  // gesture so the menu music and SFX become audible.
  const resumeAudio = () => { try { engine.context().resume() } catch (e) {} }
