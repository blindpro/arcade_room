...
`syngen-template` — a template for accessible audio experiences built on the [syngen](https://github.com/nicross/syngen) Web Audio engine. Ships as an HTML5 app (served from `public/`) and an Electron desktop app (`electron/main.js`).

No test suite, no linter, no `npm` scripts. All tasks run through Gulp.
