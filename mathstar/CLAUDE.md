...
`mathstar` — an audio-first arithmetic arcade built on the syngen-template chassis, modeled directly on `/home/games/dynamic_beatstar`. Each round the player is given an arithmetic operation at the start of a fixed-length musical window and must type the answer digit-by-digit before the music reaches the closing bar.

There is no test suite, no linter, and no `npm` scripts. All tasks run through Gulp (see "Common commands" below).

The rest of this file has two layers:
- **The next few sections** (Loop, Difficulty, Departures from beatstar, Persistence and locale, Common commands, File map) are specific to this game — read them first.
- **Everything from "Starting a new game from this template" onward** is the unchanged shared guide that ships with every game in this collection. The "Starting a new game" questionnaire does NOT apply to this repo (this game-specific CLAUDE.md exists, so the questions are settled). The architecture, audio patterns, gotchas, and conventions sections downstream all still apply.

## Loop

For each operation (every transition lands on a musical bar so the backing track stays beat-aligned):

1. `intro` (1 measure, level boundaries only) — count-in clicks plus a style/key bridge from the previous level. `content.audio.countIn()` schedules the clicks; `content.music.configure({bridgeChords, bridgeStyle, alignAt})` plays the OLD-style instrumentation on the first half-measure and the NEW-style on the second half, walking from old tonic → new V7 → new I.
2. `prep` (~½ measure) — operation reveal. `content.audio.opCue(operator)` plays a 2-note motif identifying `+`/`−`/`×`/`÷`; the announcer reads `{a} {op} {b}`. The operation is committed at this point; the player can already start typing.
3. `solve` (`2M` measures, where `M = measuresFor(level)`) — drums + bass + pad + a **continuous melodic lead** (`content.music.setLeadActive(true)`). Player types digits left-to-right; each correct digit fills a placeholder and plays a scale-degree pip via `content.audio.digit(d)` in the active key.
4. `verdict` — three pacing choices keep the music coherent:
   - **Correct**: lead stays active and the backing track keeps singing through to the next musical bar so the music never "drops" between solving and the next op cue. `enterVerdict('correct')` pins the next downbeat in `state.holdForNextOpAt`, sets `lastReason='correct'`, and the frame pump fires `enterNextOp(false, T0)` exactly on that bar (passing `T0` so the recomputed `nextDownbeat()` doesn't slip a bar forward as the scheduler advances).
   - **Wrong digit / timeout**: `M().duck(breatherDur)` ducks the bus to silence immediately so the fail buzzer + announcer ("wrong, you have N lives") sit cleanly above the music. After the 1-measure breather the music unducks back to `BUS_GAIN` and the next op begins on the next downbeat.
   - **Level clear / game over**: `VERDICT_HOLD_S` (1.4 s) pause with `content.audio.levelUp` or `content.audio.gameOver`. Game-over additionally calls `M().duck(VERDICT_HOLD_S + 0.8)` so the dirge isn't masked. `levelClear` sets `lastReason='levelClear'` so the frame pump's correct-branch doesn't double-fire enterNextOp during the setTimeout-driven hold.

Within a level, ops are queued back-to-back on bar boundaries via `content.music.nextDownbeat()`. Between levels, `pickLevelParams()` picks a fresh style + key, and `enterIntro(true)` re-runs the bridge.

Judging is digit-by-digit in `content.game.handleDigit(d)`: char-compare to `state.op.digits[state.expectedIdx]`. **First wrong digit fails the op immediately** — there is no backspace recovery. Last correct digit auto-completes (no Enter required).

## Difficulty

Read `content.math.opPool(level)` and `content.math.rangeFor(level, op)` for the source of truth. Summary:

| level | ops allowed   | numeric range                                      |
|-------|---------------|----------------------------------------------------|
| 1     | `+`           | a, b ∈ [1, 5]                                      |
| 2     | `+`           | [1, 9]                                             |
| 3     | `+ −`         | [2, 14], a ≥ b for `−`                             |
...
| 7     | `+ − ×`       | × extends to [2, 15] × [2, 9]                      |
| 8     | `+ − × ÷`     | ÷ exact (a / b ∈ [1, 12])                          |
| 9+    | `+ − × ÷`     | wider ranges                                       |

- BPM = `clamp(72 + 8·(level − 1), ≤ 168)`, then nudged into the chosen style's `bpmRange`.
- Operations to clear a level = `Math.max(3, Math.floor(level * 1.5))` — L1=3, L2=3, L3=4, L4=6, L5=7, L6=9, L7=10, L8=12, …
- Bonus life: zero misses across all ops AND average leftover-time ≥ 25%.

## Departures from beatstar

Most of `content/{audio,music,styles,theory,game}.js` is forked from `/home/games/dynamic_beatstar/src/js/content/`. Keep these intentional differences in mind when porting fixes back and forth:

- **Continuous melodic lead.** `content/music.js` adds `leadNote()`, `generateLeadPhrase()` (regenerated per measure from the current chord), and the `setLeadActive(true|false)` toggle. Beatstar's lead voice was reserved for hint patterns and silent during echo; here the lead carries the music while the player thinks. Volume is ducked under digit-pip volume so each typed digit still pops.
- **No pattern repeat.** `generatePattern` and `handleArrow` are gone. Judging path is `content.game.handleDigit(d)` — single-char compare to the next expected digit. Wrong digit → `enterVerdict('wrongDigit')`; full match → `enterVerdict('correct')`.
- **No multiplayer.** Beatstar's `mp.js` and the host/client fork in `game.js` are absent. Don't pull them in unless multiplayer becomes a real product requirement — this is a single-player, sit-down game.
- **Operator stored as i18n key.** `content.math.operatorKey(op)` returns `'op.plus' | 'op.minus' | 'op.times' | 'op.dividedBy'`. State and events store the key; the announcer renders via `app.i18n.t()` at speak-time so locale changes mid-flight don't strand a stale word. Same pattern as neverStop's `stopReasonKey` (see "i18n keys for locale-stable values" below).
- **Verdict frame-pump guard.** `content.game.frame()` only advances out of the `verdict` phase while `state.lives > 0`. Three branches dispatch by `state.lastReason`: `correct` consumes the pinned `state.holdForNextOpAt` and fires `enterNextOp(false, T0)`, `wrongDigit`/`timeout` calls `M().unduck()` (settle the bus) then `enterNextOp(false)`, and `levelClear` falls through (its hold is driven by `setTimeout`). The game-over and level-clear holds are driven by `setTimeout`, not the frame pump — without the lives guard, the breather check would re-enter `enterNextOp` mid-game-over. Without the `levelClear` lastReason sentinel, the correct-branch would also re-fire enterNextOp during the level-clear hold.
- **Lead pip = digit pip = same voice family.** `content.audio.pipForLead()` reuses beatstar's `voiceBell / voiceSquare / voicePluck / voiceMellow` keyed off the current style's `leadVoice`, plus four math-game-only timbres for the new styles: `voiceAccordion` (circus + klezmer), `voiceToyPiano` (musicBox), `voiceDrone` (dronePad), `voiceHarpsichord` (baroque). Style transitions automatically retimbre the digit cues.

### Per-style lead phrase shapes

The biggest reason the original beatstar fork all blurred together was that `generateLeadPhrase` used the same 4 measure-index variants for every style. Each style now declares a `leadShape` key (and optional `leadOctaveBias`) in `styles.js`; `generateLeadPhrase(chord, meter, measureIndex, style)` in `music.js` branches on that to pick a per-style melodic personality. One-line vibe per style:

- `lateNightJazz` (smoky) — chromatic-approach chord-tone walk; `jazzWalk` shape, walking upright bass.
- `synthwave` (neon) — ascending chord arp with off-beat colour tones; `arpUp`.
- `house` (pumping) — stab on every off-beat; `eighthStab` over four-on-the-floor kick.
- `chiptune` (8-bit) — repeating 16th-note arp on root/3rd/5th/octave; `sixteenthArp`.
- `rock` (punchy) — pentatonic riff with a hammer-on on beat 3; `pentatonic`, eighths bass.
- `waltz` (ballroom) — long downbeat note + two passing tones in 3/4; `waltz`.
- `funk` (greasy) — 16th-note syncopation off beat 1 over slap bass; `syncoSixteenths`.
- `ambient` (floating) — one note per measure, sparse and held; `sparse`.
- `latin` (tropical) — 2-3 montuno hits (1, 2-and, 3, 4-and); `montuno`, root-fifth bass.
- `disco` (glittery) — quarter-note ascending stabs hitting an octave on beat 4; `discoUp`.
- `circus` (clownish) — bouncy octave-skipping accordion lead over tuba oompah bass; `octaveBounce`.
- `cartoon` (zany) — chord tones on beats 1+2, chromatic 16th run on beats 3+4; `chromaticRun`, woodblock kit.
- `klezmer` (wedding) — chord tones with grace-note neighbour-tone approaches and a major-6th ornament; `klezmerOrnament`, accordion lead.
- `musicBox` (tinkling) — sparse high arpeggio (one note per 2 beats) in 3/4 on toy-piano; `musicBoxArp`, octave-up lead.
- `minimalPulse` (hypnotic) — Reich-style 16th repetition with phasing duck on every 5th step; `reichPhasing`.
- `dronePad` (drifting) — single sustained root tone restruck once per measure; `rootDrone`, `voiceDrone` lead.
- `clockwork` (ticking) — 16th-note pulse on a single tone with downbeat octave accent; `clockTick`, mechanical kit.
- `baroque` (studious) — continuous 8th-note stepwise counterpoint up/down chord tones; `baroqueRun`, harpsichord lead, no drums.

Bass patterns (`bassPattern` in `styles.js`) follow the same per-style logic and are independent of `bassVoice`: `walking` (jazz quarter-note walk), `oompah` (low-high tuba alternation), `rootFifth` (every-beat root/fifth), `eighths` (rock driving bass), `pulse` (root every beat), or unset (downbeat-only on the chord root).

The old generic `lounge` and `jazz` were folded into a stronger `lateNightJazz` (walking bass + jazz-walk lead). The four built-in beatstar styles (`brush`, `bossa`) are unchanged drum-kit branches; the rest of the new vibes (oompah, cartoon, klezmer, musicBox, minimal, drone, clock, baroqueNone) are math-game only.

## Scoring

Per cleared op (`enterVerdict('correct')`):

```
baseGain  = (100 + 50*digits) * OP_MULT[op]
speedMult = 0.3 + 1.1 * leftoverFrac        // 0.3 (full time used) → 1.4 (instant)
opGain    = floor(baseGain * speedMult) + 25*level
OP_MULT   = {'+': 1.0, '-': 1.25, '*': 1.75, '/': 2.5}
```

- `digits` = digits in the answer (1-3).
- `leftoverFrac` = unused fraction of the solve window, 0..1.
- Speed scales the *whole* base — dawdling is a real penalty (~0.3×), instant answers reward ~1.4×. The flat per-level bonus sits on top so high levels stay rewarding regardless of pace.
- Operator multiplier means hard ops earn more even at the same level: `8×9` at L5 is worth roughly twice a `3+4` at L1.

Plus `bonus = 500 * level` on level clear (and `lives++` on a perfect-run level — zero misses AND average leftover ≥ 25%).

When tweaking, prefer changing `OP_MULT` and the digit coefficient over the level coefficient — operator + magnitude is what makes an op feel hard, level is just a frame around them.

## Persistence and locale

- **High scores:** top 10 via `app.highscores`, dual backend. Electron writes to a JSON file via `window.ElectronApi.{readHighScores, writeHighScores}` (set up in `electron/preload.js` if/when packaging); HTML5 falls back to `localStorage['mathstar-highscores-v1']`. Modeled on `/home/games/pacman/src/js/app/highscores.js`.
- **No autosave.** `app.autosave.enable()` is intentionally not called in `main.js` — runs are sit-down sessions, lives reset to 3 every start (or `Math.max(STARTING_LIVES, level)` when level-select picks a higher start).
- **Highest-unlocked level:** `localStorage['mathstar.highestLevel']`. Plain `localStorage` (not `app.storage`) so it survives `engine.state.reset()` and is readable before `app.storage.ready()` finishes. Bumped from `pickLevelParams(level, …)` so every level the player reaches grows the unlocked range. The `levelSelect` screen reads it via `content.game.getHighestUnlocked()` and clamps the stepper.
- **Locale:** `localStorage['mathstar.lang']`. Resolution order: stored locale → `navigator.language` 2-letter prefix → `'en'`. English + Spanish dictionaries in `src/js/app/i18n.js`. Spanish operator words: `más / menos / por / entre`.
- **Global reverb send is killed** in `main.js` (`engine.mixer.reverb.setActive(false)`). Every cue authors its own ADSR + lowpass tail; we don't want the global wet bus smearing them. Belt-and-braces per the gotcha section below.

## Common commands

```sh
npm install               # first run only — populates node_modules/syngen
npx gulp build            # one-shot build of public/scripts.min.js + public/styles.min.css
npx gulp watch            # rebuild on src/** changes
npx gulp serve            # static HTTP server for public/ (port 3000)
...
npx gulp dist             # build + package electron + zip HTML5 build into dist/
```

`--debug` (e.g. `npx gulp build --debug`) skips minification and IIFE wrapping, and appends `-debug` to the version injected at the bottom of the bundle. `app.storage` strips `-debug` so debug and release share saved state.

The build artifacts `public/scripts.min.js` and `public/styles.min.css` are gitignored — never edit them.

## File map (specific to this game)

- `src/js/content/math.js` — `generate(level)` returns `{op, a, b, expr, answer, answerStr, digits}`. Per-level operation pool (`opPool`) and numeric ranges (`rangeFor`). Division is generated by picking a divisor + quotient and multiplying back, so the result is always a clean integer. Add new operations or curve tweaks here.
- `src/js/content/game.js` — top-level FSM (`idle → intro → prep → solve → verdict`), level/lives/score state, operation scheduler, judging on each keypress. Forked-and-trimmed from beatstar/game.js.
- `src/js/content/music.js` — drums + bass + pad + the new continuous lead voice. Public surface: `start`, `stop`, `configure`, `setBpm`, `setLeadActive(active)`, `duck(seconds)` / `unduck(rampS?)` (independent of start/stop fades — used by the verdict path to interrupt the music on wrong-digit/timeout and during the game-over dirge), `nextDownbeat()`, plus read-only getters `bpm/meter/style/tonality/progression`. The continuous lead is the *only* meaningful structural change vs. beatstar — drum/bass/pad code is verbatim.
- `src/js/content/audio.js` — stereo cue layer: `digit(d)`, `opCue(op)`, `correct / fail / levelUp / gameOver`, `countIn`. Voice timbres reused from beatstar's lead family.
- `src/js/content/styles.js` — 18-style registry, math-game specific. Each style declares `drumKit / bassVoice / bassPattern / padVoice / leadVoice / leadShape (+ leadOctaveBias)` so music.js can give every style a distinct sonic personality. Lounge + jazz were folded into `lateNightJazz`; new styles add circus/cartoon/klezmer/musicBox (funny) and minimalPulse/dronePad/clockwork/baroque (concentration). See "Per-style lead phrase shapes" above for the vibe-per-style summary.
- `src/js/content/theory.js` — verbatim copy from beatstar. Don't fork unless you need a new chord type.
- `src/js/app/screen/game.js` — game HUD (level/lives/score, expression, digit slots, time bar) + window-level keydown for digits + F1–F4 status hotkeys. Held-key autorepeat suppressed via per-`code` `keysDown` set. F1/F3/F5 capture-phase preventDefault'd to keep the browser's Help/Find/Reload bindings out of the way.
- `src/js/app/screen/{gameover,highscores}.js` — final score + name entry; top-10 leaderboard.
- `src/js/app/screen/levelSelect.js` — "Start at Level…" stepper, capped at `mathstar.highestLevel`. Ported verbatim from beatstar's levelSelect.
- `src/js/app/highscores.js` — the dual-backend store.
- `src/js/app/announce.js` — two ARIA live regions (polite + assertive); clear-then-set with rAF so screen readers re-read repeats.
- `src/js/app/i18n.js` — full English + Spanish dictionaries; `mt.lang` storage key. The operator words live here (`op.plus / op.minus / op.times / op.dividedBy`) and are looked up at announce-time.
- `src/css/component/game.css` — HUD layout: digit-slot pills, time bar, expression typography.
- `public/index.html` — five screens (`menu`, `game`, `gameover`, `highscores`, `language`).
