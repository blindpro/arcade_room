...
app.i18n = (() => {
  ...
  const dictionaries = {
    en: {
      ...
      'pause.title': 'Paused',
      'pause.resume': 'Resume',
      'pause.moves': 'Move list',
      'pause.restart': 'Start again',
      'pause.menu': 'Main menu',

      // Move list. Built from the roster at runtime, so these are the frames
      // the data is poured into rather than the movelist itself.
      'moves.aria': 'Move list',
      'moves.title': 'Move list',
      'moves.subtitle': 'The four buttons are the same for everyone. Each fighter changes what they weigh and which special they bring.',
      'moves.back': 'Back',
      'moves.you': 'the one you are playing',
      'moves.basics': 'The four attacks',
      'moves.attack': '{key} is {name}: {answer} {weight}',
      'moves.answerHigh': 'blocked with S,',
      'moves.answerLow': 'goes under a block, jump it,',
      'moves.punchWeight': 'short and fast.',
      'moves.kickWeight': 'long reach, heavy, slow to recover.',
      'moves.airRule': 'Anyone in the air cannot block and takes 35 percent more from a high attack, and goes down.',
      'moves.fighter': '{blurb} Health {health}, speed {speed} percent, reach {reach} percent, power {power} percent. Special: {special} - {motion}, then {button}. {note}',

      'attack.highPunch': 'high punch',
      'attack.highKick': 'high kick',
      'attack.lowPunch': 'low punch',
      'attack.lowKick': 'low kick, the sweep',

      'note.quake': 'No range limit and it cannot be blocked; jumping is the only answer.',
      'note.rush': 'Closes the gap and lands three hits high. Blockable.',
      'note.bolt': 'A projectile on the high line: block it, do not jump it.',
      'note.shadow': 'Reappears on your other side, then kicks high.',
      ...
      'help.h.specials': 'Each fighter has one special on a direction sequence, then an attack button - for example back, back, then <b>K</b>. Press <b>F3</b> in a match to hear yours and whether it is off cooldown, or pause for the full move list.',
      ...
      'help.h.pause': '<b>Escape</b> pauses, and the pause menu has a <b>move list</b> with every fighter&rsquo;s stats and special.',
      ...
      'learn.footsteps': 'Footsteps walking from your LEFT across to your RIGHT',
      'learn.rangeKick': 'KICK RANGE - one tick per step. I and K reach, U and J do not',
      'learn.rangePunch': 'PUNCH RANGE - two ticks per step. Now all four reach',
      'learn.rangeOut': 'OUT OF RANGE - no ticks. Walk in before you press anything',
    ...
    es: {
      ...
      'pause.moves': 'Lista de movimientos',
      'pause.restart': 'Empezar de nuevo',
      'pause.menu': 'Menú principal',

      'moves.aria': 'Lista de movimientos',
      'moves.title': 'Lista de movimientos',
      'moves.subtitle': 'Los cuatro botones son iguales para todos. Lo que cambia con cada luchador es su peso y el especial que trae.',
      'moves.back': 'Atrás',
      'moves.you': 'con el que juegas',
      'moves.basics': 'Los cuatro ataques',
      'moves.attack': '{key} es {name}: {answer} {weight}',
      'moves.answerHigh': 'se bloquea con S,',
      'moves.answerLow': 'pasa bajo la guardia, sáltalo,',
      'moves.punchWeight': 'corto y rápido.',
      'moves.kickWeight': 'mucho alcance, pesado, lento de recuperar.',
      'moves.airRule': 'Quien esté en el aire no puede bloquear, recibe un 35 por ciento más de un ataque alto, y cae al suelo.',
      'moves.fighter': '{blurb} Vida {health}, velocidad {speed} por ciento, alcance {reach} por ciento, potencia {power} por ciento. Especial: {special} - {motion}, y luego {button}. {note}',

      'attack.highPunch': 'puñetazo alto',
      'attack.highKick': 'patada alta',
      'attack.lowPunch': 'puñetazo bajo',
      'attack.lowKick': 'patada baja, la barrida',

      'note.quake': 'Sin límite de alcance y no se puede bloquear; saltar es la única respuesta.',
      'note.rush': 'Acorta la distancia y encadena tres golpes altos. Se puede bloquear.',
      'note.bolt': 'Un proyectil por lo alto: bloquéalo, no lo saltes.',
      'note.shadow': 'Reaparece en tu otro lado y patea alto.',
      ...
      'help.h.specials': 'Cada luchador tiene un especial con una secuencia de direcciones y luego un botón de ataque; por ejemplo atrás, atrás y <b>K</b>. Pulsa <b>F3</b> en combate para oír el tuyo y si ya está disponible, o pausa para ver la lista completa.',
      ...
      'help.h.pause': '<b>Escape</b> pausa, y el menú de pausa tiene una <b>lista de movimientos</b> con las estadísticas y el especial de cada luchador.',
      ...
      'learn.footsteps': 'Pasos cruzando de tu IZQUIERDA a tu DERECHA',
      'learn.rangeKick': 'ALCANCE DE PATADA: un tic por paso. I y K llegan; U y J no',
      'learn.rangePunch': 'ALCANCE DE PUÑO: dos tics por paso. Ya llegan los cuatro',
      'learn.rangeOut': 'FUERA DE ALCANCE: sin tics. Acércate antes de atacar',
