app.screenManager = (() => {
  ...
  const machine = engine.tool.fsm.create({
    ...
    transition: {
      none: {
        activate: function () {
          // Honor #hash diagnostic routes (e.g. #test, #learn, #help). The
          // 'none → activate' transition is the only place this is safe —
          // by the time main.js dispatches the FSM is past 'none'.
