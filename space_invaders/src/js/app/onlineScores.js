 ...
 *
 * Drop-in module: opens a session on game start, posts the score with
 * the player's name on game over, returns server rank.
 *
 * Cross-cutting goals:
 *   - Never let a network error crash the run; submit() throws,
 *     gameover handles it.
 *   - Single-use token: drop it after submit so the next run opens a
 *     fresh session.
 */
app.onlineScores = (() => {
  const BASE = 'https://scores.oriolgomez.com'
  const GAME_ID = 'spaceinvaders'
  const SECRET = '1YVF83ik4pO7ZK9kB2lC0xlQaPJfKDkuQo5r6yocIGU'
