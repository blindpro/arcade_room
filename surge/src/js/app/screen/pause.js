app.screen.pause = app.screenManager.invent({
  ...
  transitions: {
    resume: function () { content.game.resume(); this.change('game') },
    restart: function () {
      const mode = content.game.state() && content.game.state().mode || 'practice'
      content.game.start(mode)
      this.change('game')
    },
  ...
  onFrame: function () {
    try {
      ...
      if (ui.back || ui.pause) {
        ...
        content.game.resume()
        app.announce.assertive(app.i18n.t('ann.resumed'))
