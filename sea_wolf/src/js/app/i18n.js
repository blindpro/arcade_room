...
app.i18n = (() => {
  ...
  const dictionaries = {
    en: {
      ...
      'help.h.sweep': '<kbd>F</kbd> - a quick hydrophone sweep, and the opposite of the ping in every way. You get one short beep for every ship in earshot, each placed at roughly that ship&rsquo;s direction, arriving as a burst that swings clockwise round the boat from dead ahead. So the number of beeps is how many ships are out there, and where each one sits in the field is roughly where that ship is - bright forward of the beam, an octave lower once it is behind you, and harsher for an escort, exactly like the contact beeps. It answers instantly, it costs you no noise at all because you are only listening, and you can use it about once a second. In exchange the bearings are rough - smeared by anything up to twenty-odd degrees, nowhere near tight enough to shoot on - and the beeps carry no range at all; for that, read the spoken report, which gives each contact as close, middle distance or far off. Use the sweep to know what is around you and which way to come round; use the ping when you need a number. Going deep muffles it, like everything else the passive set hears.',
      ...
      'learn.sweep': 'Hydrophone sweep - four ships, one beep each',
    ...
    es: {
      ...
      'help.h.sweep': '<kbd>F</kbd> - una escucha rapida del hidrofono, y lo contrario del ping en todo. Recibes un pitido corto por cada barco al alcance, colocado aproximadamente en la direccion de ese barco, en una rafaga que gira en el sentido de las agujas del reloj desde la proa. Asi que el numero de pitidos es cuantos barcos hay, y donde cae cada uno en el campo es mas o menos donde esta ese barco: agudo por delante del traves, una octava mas grave en cuanto queda por la popa, y mas aspero si es una escolta, igual que los pitidos de contacto. Responde al instante, no te cuesta nada de ruido porque solo estas escuchando, y puedes usarla mas o menos una vez por segundo. A cambio las demoras son aproximadas (difuminadas hasta unos veinte grados, ni de lejos lo bastante finas para disparar) y los pitidos no llevan distancia alguna; para eso esta el informe hablado, que da cada contacto como cerca, a media distancia o lejos. Usala para saber que tienes alrededor y hacia donde virar; usa el ping cuando necesites un numero. Bajar de cota la amortigua, como a todo lo que oye el equipo pasivo.',
      ...
      'learn.sweep': 'Escucha del hidrofono: cuatro barcos, un pitido cada uno',
