app.screen.gameover = app.screenManager.invent({
  ...
  onEnter: function () {
    ...
    const sunk = content.game.state.sunk
  ...
  handleSave: function () {
    ...
    if (this.state.qualifies) app.highscores.add(name, score, sunk)
    ...
    Promise.resolve(app.onlineSubmit.run({
      name: name, score: score, meta: {sunk: sunk},
